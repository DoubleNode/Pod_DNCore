...
#import "UIView+DNCPending.h"

const NSString* kDNCPendingAlpha        = @"DNCPendingAlpha";
const NSString* kDNCPendingFrame        = @"DNCPendingFrame";
const NSString* kDNCPendingTransform    = @"DNCPendingTransform";
...
{
    objc_setAssociatedObject(self, &kDNCPendingAlpha, [NSNumber numberWithFloat:pendingAlpha], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (float)pendingAlpha
{
    return [objc_getAssociatedObject(self, &kDNCPendingAlpha) floatValue];
}

- (void)setPendingFrame:(CGRect)pendingFrame
{
    objc_setAssociatedObject(self, &kDNCPendingFrame, [NSValue valueWithCGRect:pendingFrame], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (CGRect)pendingFrame
{
    return [objc_getAssociatedObject(self, &kDNCPendingFrame) CGRectValue];
}

- (void)setPendingTransform:(CATransform3D)pendingTransform
{
    objc_setAssociatedObject(self, &kDNCPendingTransform, [NSValue valueWithCATransform3D:pendingTransform], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (CATransform3D)pendingTransform
{
    return [objc_getAssociatedObject(self, &kDNCPendingTransform) CATransform3DValue];
}
