...
@implementation DNCDate

+ (id)dateWithComponentFlags:(unsigned)flags fromDate:(NSDate*)date
{
    return [[DNCDate alloc] initWithComponentFlags:flags fromDate:date];
}

+ (id)dateWithComponents:(NSDateComponents*)components
{
    return [[DNCDate alloc] initWithComponents:components];
}
...
{
    DNCDate*    otherDate   = (DNCDate*)otherObject;
